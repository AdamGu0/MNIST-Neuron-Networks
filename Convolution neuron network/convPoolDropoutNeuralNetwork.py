...
def layer_forward(A_prev, W, b, activation, dropout=True):
    '''
    ...
    if dropout:
        p = 0.5
        P = np.random.rand(A.shape[0], A.shape[1])
        P = P < p
        A = A * P / p
        cache["P"] = P
        cache["p"] = p
...
def multi_layer_forward(X, parameters, dropout=True):
    '''
    ...
    for l in range(1, L):  # since there is no W0 and b0
        A, cache = layer_forward(A, parameters["W" + str(l)], parameters["b" + str(l)], "relu", dropout)
        caches.append(cache)

    AL, cache = layer_forward(A, parameters["W" + str(L)], parameters["b" + str(L)], "linear", dropout)
    caches.append(cache)
    return AL, caches
...
def classify(X, parameters, convParameters):
    ...
    AL, caches = multi_layer_forward(A0, parameters, dropout=False)
