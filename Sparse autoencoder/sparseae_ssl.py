...
def softmax_classify(X, parameters):
    '''
    Network prediction for inputs X
    ...
    W2 = parameters["W2"]
    b2 = parameters["b2"]
    A1, cache1 = layer_forward(X, W1, b1)
    A2, cache2 = linear_forward(A1, W2, b2)
    A, cache, cost = softmax_cross_entropy_loss(A2)
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.2, decay_rate=0.01):
    ...
    for ii in range(num_iterations):
        ...
        A2, cache2 = linear_forward(A1, W2, b2)
        ## call loss function
        A, cache, cost = softmax_cross_entropy_loss(A2, Y)

        # Backward Prop
        # loss der
        dA2 = softmax_cross_entropy_loss_der(Y, cache)
        ## call to layer_backward to get gradients
        gradients = {}
        dA1, gradients["dW2"], gradients["db2"] = linear_backward(dA2, cache2, W2, b2)
        dA0, gradients["dW1"], gradients["db1"] = layer_backward(dA1, cache1, W1, b1)
...
def semi_supervised_learning(X, Y, net_dims, parameters, num_iterations=500, learning_rate=0.2, decay_rate=0.01):

    tmp = initialize_multilayer_weights(net_dims)
    parameters["W2"] = tmp["W1"]
    parameters["b2"] = tmp["b1"]
    costs = []
...
def main():
    ...
    costs, parameters = train_sparse_autoencoder(train_data, train_data, net_dims, num_iterations=num_iterations,
    ...
    costs, parameters = semi_supervised_learning(train_data, train_label, net_dims, parameters,
    ...
    plt.title("Sparse autoencoder network: " + str(net_dims) + " (p = " + str(p) +
    ...
    costs, parameters = multi_layer_network(train_data, train_label, net_dims, num_iterations=num_iterations,
